...
# Define paths for model, checkpoint, engines, and repository
...
if [ ! -d "$ENGINE_PATH" ] || [ ! -d "$VISUAL_ENGINE_PATH" ]; then
    ...
    # Convert HF checkpoint to TRT-LLM format with BF16 precision
    ...
    # Build LLM engine with BF16 precision
    "trtllm-build" \
...
# Create engine subdirectories and copy specific engine files
